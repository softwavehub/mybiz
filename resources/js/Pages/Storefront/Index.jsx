...
import { ShoppingBag, Truck, ShieldCheck, Star, Sparkles } from 'lucide-react';

export default function StorefrontIndex({ merchant, products }) {
  const storeName = merchant?.store_name || 'UrbanStyle Apparel';
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productList.map((p) => (
            <div key={p.id} className="group bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-indigo-500/60 transition-all duration-300 flex flex-col justify-between hover:shadow-2xl hover:shadow-indigo-500/20">
              <div>

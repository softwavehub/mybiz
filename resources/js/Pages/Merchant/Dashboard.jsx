...
import { Store, Plus, Lock, Globe, DollarSign, Tag, TrendingUp } from 'lucide-react';

export default function MerchantDashboard({ merchant, catalog }) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {(catalog || [
          ...
          const retail = Math.round(item.floor * (1 + markupPercent / 100));
          const profit = Math.round(retail - item.floor);

          return (
            <div key={item.id} className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between">
              <div>
              ...
              <div className="mt-5 border-t border-slate-800/80 pt-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <span className="text-lg font-extrabold text-white">₹{retail}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-xs text-slate-500 block">Your Profit</span>
                    <span className="text-base font-extrabold text-emerald-400">+₹{profit}</span>
                  </div>
                </div>
